...
def build_model(nodes, activation, optimizer):
  ...
  if (optimizer == 'adam'):
    optimizer = tf.keras.optimizers.Adam(lr=0.01) # Uncomment to use Adam as optimizer
  else:
    optimizer = tf.keras.optimizers.SGD(lr=0.01, momentum=0.1) # Uncomment to use SGD as optimizer
  ...
  for activation in activations:
    for node in range(1, nodes):
      for i in range(0, attempts):
        ...
        new_row = {'id': identifier, 'optimizer': optimizer, 'learning_rate': 0.01, 'activation': activation, 'epoch': 6000, 'configuration': ('11-' + str(node) + '-1'), 'train': training_rmse, 'val': validation_rmse, 'data': 'all'}

        print(new_row)

        should_save = (result.loc[(identifier), 'val'] > validation_rmse) if (len(result.index) >= (identifier + 1)) else True
        ...
        if (should_save):
          ...
          if (new_model):
            result = result.append(pd.DataFrame(new_row, index=[identifier]))
          else:
            result.update(pd.DataFrame(new_row,index=[identifier]))
